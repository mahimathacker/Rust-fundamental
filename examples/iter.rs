...
fn main() {
    ...
    let v3: Vec<(String, u32)> = vals1.iter().map(|v| (v.0.to_string(), v.1 + 1)).collect();

    println!("vals: {:?}", v3);

    let v4: HashMap<String, u32> = vals1.iter().map(|v| (v.0.to_string(), v.1 + 1)).collect();
    ...
    let filter_vals: Vec<u32> = vals
        .into_iter()
        .filter(|x: &u32| *x <= 3)
        .map(|x: u32| x + 1)
        .collect(); //Keep one refrence and destructing because into_iter takes only T and not &T unlike the iter()
    println!("filter_vals: {:?}", filter_vals);
}
